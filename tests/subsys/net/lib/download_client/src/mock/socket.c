...
{
	printk("mock_socket_offload_recvfrom\n");
	k_sleep(K_MSEC(50));
...
{
	printk("mock_socket_offload_read\n");
...
{
	printk("mock_socket_offload_sendto %d\n", len);
	k_sleep(K_MSEC(50));
	return ztest_get_return_value();
}
...
static int mock_socket_offload_ioctl(void *obj, unsigned int request, va_list args)
{
	...
	case ZFD_IOCTL_POLL_PREPARE:
		printk("mock_socket_offload_ioctl, ZFD_IOCTL_POLL_PREPARE\n");
		k_sleep(K_MSEC(50));
		return -EXDEV;

	case ZFD_IOCTL_POLL_UPDATE:
		printk("mock_socket_offload_ioctl, ZFD_IOCTL_POLL_UPDATE\n");
		k_sleep(K_MSEC(50));
		return -EOPNOTSUPP;

	case ZFD_IOCTL_POLL_OFFLOAD: {
		printk("mock_socket_offload_ioctl, ZFD_IOCTL_POLL_OFFLOAD\n");
		k_sleep(K_MSEC(50));
		return 0;
	}

	case ZFD_IOCTL_SET_LOCK: {
		printk("mock_socket_offload_ioctl, ZFD_IOCTL_SET_LOCK\n");
		k_sleep(K_MSEC(50));
		return 0;
	}
	 ...
	 */
	default:
		printk("mock_socket_offload_ioctl, mock_socket_offload_fcntl\n");
		k_sleep(K_MSEC(50));
		return 0;
	}
...
{
	printk("mock_socket_offload_bind\n");
	k_sleep(K_MSEC(50));
	return 0;
}
...
{
	printk("mock_socket_offload_connect\n");
	k_sleep(K_MSEC(50));
	return 0;
}
...
{
	printk("mock_socket_offload_listen\n");
	k_sleep(K_MSEC(50));
	return 0;
}
...
{
	k_sleep(K_MSEC(50));
	printk("mock_socket_offload_accept\n");
	return 0;
}
...
{
	k_sleep(K_MSEC(50));
	printk("mock_socket_offload_sendmsg\n");
	return 0;
}
					  ...
					  socklen_t optlen)
{
	...
	k_sleep(K_MSEC(50));
	printk("mock_socket_offload_setsockopt\n");

	if ((level == SOL_SOCKET) && ((optname == SO_RCVTIMEO) || (optname == SO_SNDTIMEO))) {
		time.tv_sec = ((struct timeval *)optval)->tv_sec;
		time.tv_usec = ((struct timeval *)optval)->tv_usec;

		printk("mock_socket_offload_setsockopt tv_sec %lld, tv_usec %d\n", time.tv_sec,
		       time.tv_usec);
	}
...
{
	k_sleep(K_MSEC(50));
	printk("mock_socket_offload_getsockopt\n");
	return 0;
}
